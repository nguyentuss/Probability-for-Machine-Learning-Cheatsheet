...
\section{Linear Models}\smallskip \hrule height 2pt \smallskip
\subsection{Linear Discriminant Analysis}
Consider classification models of the following form
\[
    p(y=c\mid\inp,\parameter) = \frac{p(\inp|y=c,\parameter)p(y=c|\parameter)}{\sum_{c'}p(\inp|y=c',\parameter)p(y=c'|\parameter)}
\]
where $p(y=c|\parameter)$ is the prior distribution, and $p(\inp|y=c,\parameter)$ is the class conditional density for class c. 
...
%     \item Now suppose we measure performance in terms of the average misclassification rate on the training set:
%           \begin{equation*}
%               R(M) = \frac{1}{n} \sum_{i=1}^{n} \mathbb{I}(y_i \neq \hat{y}(x_i))
%           \end{equation*}
